...
Current datasets:
    ...
    4. employee
    5. housing
    6. student_performance

Usage:
    cd E:\\nl2sql_cse366\\benchmark
    python -m benchmark.run_benchmark

IMPORTANT:
    Dataset configuration is loaded ONLY from:
        benchmark/config.py
"""
...
# This file:
#
# E:\nl2sql_cse366\benchmark\run_benchmark.py
#
# Project root:
#
# E:\nl2sql_cse366
...
BENCHMARK_PROJECT_DIR = os.path.dirname(
...
# Benchmark package:
#
# E:\nl2sql_cse366\benchmark\benchmark
#
...
# Project root is required for core imports
...
try:
    ...
    from benchmark.config import (
        DATASETS as CONFIG_DATASETS
    )

except Exception as e:

    print()
    print("ERROR: Could not load benchmark/config.py")
    print(f"Reason: {e}")
    print()
    sys.exit(1)
...
#
# IMPORTANT:
# DO NOT hardcode dataset CSV names here.
#
...
# ============================================================
#
# Allows old question naming to remain compatible for the
# five datasets whose names changed.
#
# IMPORTANT:
# temp_and_rain_dataset is intentionally NOT included.
#
# If questions.json still contains temp_and_rain_dataset,
# the benchmark will stop and tell you to replace those
# 30 questions with diabetes questions.
#
# ============================================================
...
def normalize_dataset_key(
    ...
    # Explicitly detect the removed dataset
    if key.lower() in {
        "temp_and_rain",
        "temp_and_rain_dataset",
    }:
        ...
        raise ValueError(
            "Dataset 'temp_and_rain_dataset' is no longer "
            "supported. Replace its 30 questions in "
            "questions.json with the diabetes dataset."
        )
...
def get_table_for_dataset(
    ...
    if canonical_key not in DATASETS:

        raise KeyError(
            f"Dataset '{canonical_key}' not found in config.py"
        )
...
def validate_dataset_configuration():
    ...
    print(
        f"       [OK] "
        f"{actual_dataset_count} unique datasets"
    )
...
def benchmark_query(
    ...
    try:
        ...
        table_name = get_table_for_dataset(
            dataset_key
        )
    ...
    expected_intent = normalize_intent(
...
def main():
    ...
    if (
        ...
        print(
            "[WARNING] Benchmark configuration mismatch."
        )
