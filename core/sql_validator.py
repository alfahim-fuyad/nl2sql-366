...
Performs safety checks to block injection attacks and data modification
commands, and correctness checks to verify table and column references.
"""
...
def _strip_string_literals(sql):
    ...
    This prevents false positives when a dangerous keyword appears inside
    a user data value (e.g. WHERE notes = 'drop box').
    """
    return re.sub(r"'(?:[^']|'')*'", "''", sql)
...
def validate_sql(sql, schema, table_name="data"):
    """
    Validate a SQL query string before execution.

    Checks performed:
        1. Query must start with SELECT.
        2. No dangerous data-modification keywords (after stripping literals).
        3. No semicolons within the query body (blocks multi-statement injection).
        4. Correct table name referenced in FROM / JOIN clause.
        5. All columns in the WHERE clause exist in the schema.
    ...
    # Rule 2: Dangerous keywords (checked on literal-stripped SQL)
    ...
    # Rule 3: No extra semicolons (multi-statement injection)
    if ";" in sql_stripped.rstrip(";"):
        return False, "Multiple SQL statements are not allowed."

    # Rule 4: Correct table name
    ...
    # Rule 5: WHERE columns must exist in schema
